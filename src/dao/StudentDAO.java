...
public class StudentDAO {
    ...
    public List<Student> getStudentPage(int offset, int limit) {
    List<Student> students = new ArrayList<>();
    try {
        ...
        while (rs.next()) {
            students.add(new Student(
                rs.getString("cardID"), rs.getString("name"),
                rs.getString("NIM"), rs.getString("studiProgram")
            ));
        }
    } catch (SQLException e) {
        e.printStackTrace();
    ...
    public int getTotalCount() {
        int total = 0;
        try {
             PreparedStatement stmt = connection.prepareStatement("SELECT COUNT(*) AS total FROM students");
             ResultSet rs = stmt.executeQuery();
    ...
    public List<Student> searchByNim(String keyword, int offset, int limit) {
    List<Student> students = new ArrayList<>();
    try {
        String sql = "SELECT * FROM students WHERE NIM LIKE ? LIMIT ? OFFSET ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
...
public int getSearchCount(String keyword) {
    int total = 0;
    try {
        PreparedStatement stmt = connection.prepareStatement("SELECT COUNT(*) AS total FROM students WHERE NIM LIKE ?");
        stmt.setString(1, "%" + keyword + "%");
        ResultSet rs = stmt.executeQuery();
        if (rs.next()) total = rs.getInt("total");
    } catch (SQLException e) {
